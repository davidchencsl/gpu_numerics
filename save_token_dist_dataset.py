...
Output format: JSONL (one line per sampled item).
"""
import os
import argparse
import json
...
def build_prompt_from_record(rec: dict, template: str = DEFAULT_TEMPLATE) -> Tuple[str, List[str]]:
    """
    Tries to robustly extract question + options from an MMLU-Pro record.
    Returns (prompt_text, option_labels_list).
    """
    # Common field names seen in public releases
    question = rec.get("question") or rec.get("stem") or rec.get("prompt") or ""
    options = (
        rec.get("options")
        or rec.get("choices")
        or rec.get("answers")  # sometimes 'answers' holds option texts, not just the gold
        or rec.get("candidate_answers")
    )

    # Some variants put options in dict with keys "A","B","C","D"
    if options is None:
...
def main():
    ...
    parser.add_argument("--output", required=True, help="Output JSONL file (one record per sampled item).")
    ...
    parser.add_argument("--prompt_template_file", default=None,
                        help="Optional path to a custom Jinja-like template with {question} and {options_block}.")
    ...
    if out_path.exists():
        print(f"[WARN] Output file exists and will be appended: {out_path}")
    ...
    for count, idx in enumerate(indices, start=1):
        ...
        dump_jsonl(out_path, obj)
        print(f"[{count}/{args.n_samples}] wrote item idx={idx} | seq_len={seq_len} | top-{k}")
